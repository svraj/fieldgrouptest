...
public class TimingsProcessingServiceImpl implements TimingsProcessingService {
	...
	private void setDayBalance(ProcessedTiming processedTiming) throws TmsBusinessException {
		...
		if (processedTiming.getTotalWorkingHours() != null) {
			Duration dayBalance = processedTiming.getTotalWorkingHours()
					.minus(processedTiming.getTimingProfile().getDailyWorkHours());
	...
	private void setOvertimeCalculations(ProcessedTiming processedTiming) throws TmsBusinessException {
		...
		if (processedTiming.getTimingProfile().getOvertimeAllowed() != null
				&& processedTiming.getTimingProfile().getOvertimeAllowed()) {
	...
	private void setIdealTimeOut(ProcessedTiming processedTiming) throws TmsBusinessException {
		...
		if (processedTiming.getInDateTime() != null) {
			...
			if (processedTiming.getTotalBreakDuration() != null) {
				Duration overflownBreakHours = processedTiming.getTotalBreakDuration()
						.minus(timingProfile.getMinBreakDuration());
	...
	private void setHourCalculations(ProcessedTiming processedTiming) {
		if (processedTiming.getInDateTime() != null && processedTiming.getOutDateTime() != null) {
	...
	@Override
	public BreakDetail calculateAndUpdateBreakDetail(BreakDetail breakDetail, TimingProfile timingProfile)
			throws TmsBusinessException {
	...
	public TimingsProcessDTO processProcessedTimingsForEmployee(Employee employee, DateTime startDate,
			DateTime endDate) {
		// TODO
